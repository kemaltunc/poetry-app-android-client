...
import com.tunc.androidpoetryapp.customviews.FragmentView.Companion.currentController
...
class FragmentView @JvmOverloads constructor(
    ...
    fun navigate(fragment: Fragment, popup: Fragment? = null, noHistory: Boolean = false) {
        transaction(fragment, popup, supportFragmentManager, noHistory)
    }

    fun childNavigate(fragment: Fragment, popup: Fragment? = null, noHistory: Boolean = false) {
        transaction(fragment, popup, currentFragment()?.childFragmentManager, noHistory)
    }
    ...
    private fun transaction(
        ...
        val popupName = popup?.javaClass?.name
        ...
        fragmentManager?.let {
            if (fragmentName != null) {
                ...
                popup?.let {
                    ...
                    bundle.putString(POPUP_FRAGMENT, popupName)
                    fragment.arguments = bundle
                }
                if (noHistory) removeHistory(fragmentManager)
    ...
    private fun removeHistory(fragmentManager: FragmentManager) {
        ...
        val currentFragment = currentFragment(fragmentManager)

        currentFragment?.let {
            fragmentManager.beginTransaction().remove(it).commit()
            popStackInclusive(fragmentManager, it.javaClass.name)
        }
    }
